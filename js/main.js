...
// ---Function for Rendering Popular Page---
...
function getPopular() {
  ...
  xhr.addEventListener('load', function () {
    for (let i = 0; i < xhr.response.data.length; i++) {
      const $popular = renderPopular(xhr.response.data[i]);
      $popularrow.appendChild($popular);
    }
  });
  xhr.send();
}
...
// ---Function for Rendering Upcoming Page---
...
function getUpcoming() {
  ...
  xhr.addEventListener('load', function () {
    for (let i = 0; i < xhr.response.data.length; i++) {
      const $upcoming = renderUpcoming(xhr.response.data[i]);
      $upcomingrow.appendChild($upcoming);
    }
  });
  xhr.send();
}
